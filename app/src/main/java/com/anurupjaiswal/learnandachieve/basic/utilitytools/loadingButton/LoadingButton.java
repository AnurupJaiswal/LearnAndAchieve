 ...
 *
 */
package com.anurupjaiswal.learnandachieve.basic.utilitytools.loadingButton;
...
public class LoadingButton extends DrawableTextView {
    ...
    int loadingDrawableSize;
    int initialCornerRadius;
    int loadingDrawableColor;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        enableRestore = array.getBoolean(R.styleable.LoadingButton_enableRestore, true);
        mShrinkDuration = array.getInt(R.styleable.LoadingButton_shrinkDuration, 450);
        mShrinkShape = array.getInt(R.styleable.LoadingButton_shrinkShape, ShrinkShape.OVAL);

        loadingDrawableSize = array.getDimensionPixelSize(R.styleable.LoadingButton_loadingEndDrawableSize, (int) (enableShrink ? getTextSize() * 2 : getTextSize()));
        ...
        if (mLoadingPosition == 0) {
            setDrawable(mLoadingPosition, mLoadingDrawable, loadingDrawableSize, loadingDrawableSize, dp2px(loadingDrawableSize / 2 - getCompoundDrawablePadding() / 2));
        } else {
    ...
    private void setUpShrinkAnimator() {
        ...
        mShrinkAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (curStatus == 1) {
                    if ((float) animation.getAnimatedValue() > ((float) initialCornerRadius / (float) (getLayoutParams().height / 2))) {
                        setRadius((int) ((float) animation.getAnimatedValue() * (getLayoutParams().height / 2)));
                    }
                } else {
                    int value = (int) ((float) initialCornerRadius + (((float) (getLayoutParams().height / 2) - (float) initialCornerRadius) * (float) animation.getAnimatedValue()));
                    setRadius(value);
                }
                requestLayout();
            }
     ...
     */
    public void complete(Activity activity, boolean isSuccess) {
