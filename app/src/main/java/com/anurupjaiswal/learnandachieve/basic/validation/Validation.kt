package com.anurupjaiswal.learnandachieve.basic.validation
...
import com.anurupjaiswal.learnandachieve.basic.utilitytools.SavedData.getCountryKey
...
class Validation {
    ...
    fun CheckValidation(
        ...
        for (validationModel in errorValidationModels) {
            ...
            when (validationModel.type) {
                ...
                Type.PinCode -> validationCheck =isValidPincode(context,validationModel.editText)
    ...
    private fun isValidPincode(context: Context, editText: EditText?): Boolean {
        return if (editText?.text == null || TextUtils.isEmpty(editText.text)) {
            errorMessage = context.getString(R.string.empty_error) // Message for empty input
            EditTextPointer = editText
            false
        } else {
            val p = Pattern.compile("[0-9]{6}") // Regex for 6-digit numeric PIN code
            val m = p.matcher(editText.text.toString())
            if (m.matches()) {
                true
            } else {
                EditTextPointer = editText
                errorMessage = context.getString(R.string.invalid_pincode_error) // Message for invalid PIN code
                false
            }
        }
    }
    ...
    enum class Type(var label: String) {
        ...
        PAN(""), IFSC(""), Empty(""), PinCode(""), AccountNumber(""),GST(""), PasswordLength("");
    }
